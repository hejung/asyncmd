...
class Test_Trajectory(TBase):
    ...
    def test_init_raises_non_existing_files(self, trajectory_files, structure_file):
        with pytest.raises(FileNotFoundError):
            _ = Trajectory(
                trajectory_files=trajectory_files, structure_file=structure_file
            )

    @pytest.mark.parametrize(
        ["traj_file", "struct_file", "truth"],
        [
            (
    ...
    def test_binary_magic_methods(self, traj_files, struct_files, truth):
        traj1 = Trajectory(
            trajectory_files=traj_files[0],
            structure_file=struct_files[0],
        )
        traj2 = Trajectory(
            trajectory_files=traj_files[1],
    ...
    @pytest.mark.parametrize("cache_type", ["npz", "h5py", "memory"])
    @pytest.mark.parametrize("initial_cache_type", ["npz", "h5py", "memory"])
    def test__setup_cache(self, tmp_path, cache_type, initial_cache_type):
        ...
        func_values = [
            self.make_func_values(traj_len=200, cv_dim=cv_dim) for cv_dim in cv_dims
        ]
    ...
    @pytest.mark.parametrize("cache_type", ["npz", "h5py", "memory"])
    @pytest.mark.parametrize("change_wdir_between_pickle_unpickle", [True, False])
...
class Test_TrajectoryFunctionValueCache(TBase):
    ...
    def test_append_iter_len___getitem___errs(self, tmp_path, cache_class):
        ...
        test_data_func_values = [
            self.make_func_values(traj_len, cv_dim) for cv_dim in cv_dims
        ]
        ...
        for func_id in cache:
            ...
            assert np.all(
                np.equal(cache[func_id], test_data_func_values[idx_in_test_data])
            )
        assert len(cache) == n_cached_cvs
        ran_idx = self.ran_gen.integers(n_cached_cvs)
        assert np.all(
            np.equal(cache[test_data_func_ids[ran_idx]], test_data_func_values[ran_idx])
        )
    ...
    def test_stateful_append_iter_len(self, tmp_path, cache_class):
        ...
        n_initial_cvs = 2
        traj_len = 123
        cv_dims = [1] + [self.ran_gen.integers(300) for _ in range(n_cached_cvs - 1)]
        test_data_func_ids = [self.make_func_id() for _ in range(n_cached_cvs)]
        test_data_func_values = [
            self.make_func_values(traj_len, cv_dim) for cv_dim in cv_dims
        ]
        # now create a fresh cache and append
        cache = cache_class(traj_hash=traj_hash, traj_files=traj_files)
        for func_id, func_values in zip(
            test_data_func_ids[:n_initial_cvs], test_data_func_values[:n_initial_cvs]
        ):
            cache.append(func_id=func_id, values=func_values)
        ...
        for func_id in cache2:
            idx_in_test_data = test_data_func_ids.index(func_id)
            assert np.all(
                np.equal(cache2[func_id], test_data_func_values[idx_in_test_data])
            )
    ...
    def test__ensure_consistent_npz(self, tmp_path):
        ...
        npz_cache_mm = TrajectoryFunctionValueCacheInNPZ(
            traj_hash=hash_traj_mm, traj_files=traj_files
        )
        ...
        _ = TrajectoryFunctionValueCacheInNPZ(
            traj_hash=traj_hash, traj_files=traj_files
        )
        assert not os.path.exists(cache_file_name)
    ...
    def test_h5py_multiple_cache_sources(self, tmp_path, copy_h5py, caplog):
        ...
        # append it
        cache = TrajectoryFunctionValueCacheInH5PY(
            traj_hash=traj_hash, traj_files=traj_files
        )
        for func_id, func_values in zip(
            test_data_func_ids[:n_initial_cvs], test_data_func_values[:n_initial_cvs]
        ...
        with caplog.at_level(logging.ERROR):
            cache.append(
                func_id=test_data_func_ids[n_initial_cvs + 1],
        ...
        asyncmd.config.register_h5py_cache(
            h5py_group=h5file_r_with_data, copy_h5py=False
        )
        cache = TrajectoryFunctionValueCacheInH5PY(
            traj_hash=traj_hash, traj_files=traj_files
        )
    ...
    def test_TrajectorySingleton(self):
        """Test that Trajectory implements singleton pattern for same files."""
        # Test 1: Same files should return same object
        traj1 = Trajectory(
            trajectory_files="tests/test_data/trajectory/ala_traj.trr",
            structure_file="tests/test_data/trajectory/ala.tpr",
        )
        ...
        assert traj1 is traj2, "Same trajectory files should return identical object"

        # Test 2: Same object has same hash
        assert traj1.trajectory_hash == traj2.trajectory_hash

        # Test 3: Changes to one affect the other (since they're the same)
        traj1.nstout = 100
        assert traj2.nstout == 100, "Modifications to one instance affect the singleton"

        # Test 4: Different files return different objects
        traj3 = Trajectory(
            trajectory_files="tests/test_data/trajectory/ala_conf_in_no_state.trr",
            structure_file="tests/test_data/trajectory/ala.tpr",
        )
        assert (
            traj1 is not traj3
        ), "Different trajectory files should return different objects"

        # Test 5: Explicitly test cache type change propagates to singleton
        asyncmd.config.set_trajectory_cache_type(cache_type="memory", copy_content=True)
        # traj1 is still the same object, so it should now use memory cache
        assert isinstance(traj1._cache, TrajectoryFunctionValueCacheInMemory)

        # Test 6: Clearing trajectories allows recreation
        asyncmd.trajectory._forget_all_trajectories()
        traj4 = Trajectory(
            trajectory_files="tests/test_data/trajectory/ala_traj.trr",
            structure_file="tests/test_data/trajectory/ala.tpr",
        )
        assert traj1 is not traj4, "After forgetting trajectories, new object created"
        assert traj4.trajectory_hash == traj1.trajectory_hash

    def test_OneH5PYGroupTrajectoryFunctionValueCache_raises_on_read_only(
        self, tmp_path
    ):
        h5py = pytest.importorskip(
            "h5py",
        ...
        h5file = h5py.File(fname_traj_cache, mode="w")
        root_grp = h5file.require_group(
            f"asyncmd/TrajectoryFunctionValueCache/{traj_hash}"
        )
        ...
        cache = OneH5PYGroupTrajectoryFunctionValueCache(
            traj_hash=traj_hash, traj_files=traj_files, h5py_cache=h5file
        )
